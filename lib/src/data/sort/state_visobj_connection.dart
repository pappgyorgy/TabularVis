...
class StateVisObjConnection extends State{
  ...
  StateVisObjConnection(SortDataSearchAlgorithm diagramElements) : super._(diagramElements){
  ...
  void setupNeighbourHelpers([int startRange = 1, int endRange = -1]){
    ...
    for(int neighbour = startRange; neighbour < endRange; neighbour++){
      ...
      this.groupToMoveAndWhereToMove[neighbour][2] = remainder ~/ m + 1;
  ...
  StateVisObjConnection.simpleCopy(State object){
    ...
    this.allGroupsOneBlock = (object as StateVisObjConnection).allGroupsOneBlock;

    this.maxBlockNumberInGroup = (object as StateVisObjConnection).maxBlockNumberInGroup;

    this._numberOfNeighbour = (object as StateVisObjConnection)._numberOfNeighbour;

    //this.neighboursValues = new List<int>(this._numberOfNeighbour + 1);

    this.posDoubleConvertValue = (object as StateVisObjConnection).posDoubleConvertValue;

    this._convertOrderToRadian = (object as StateVisObjConnection)._convertOrderToRadian;
    ...
    this.groupChildrenNumber = (object as StateVisObjConnection).groupChildrenNumber.toList(growable: false);

    for(int neighbour = 1; neighbour < this._numberOfNeighbour; neighbour++) {
      this.groupToMoveAndWhereToMove[neighbour][0] = (object as StateVisObjConnection).groupToMoveAndWhereToMove[neighbour][0];
      this.groupToMoveAndWhereToMove[neighbour][1] = (object as StateVisObjConnection).groupToMoveAndWhereToMove[neighbour][1];
      this.groupToMoveAndWhereToMove[neighbour][2] = (object as StateVisObjConnection).groupToMoveAndWhereToMove[neighbour][2];
    }

    this.neighboursValues = (object as StateVisObjConnection).neighboursValues.toList(growable: false);

    this.finalOrderValue = (object as StateVisObjConnection).finalOrderValue;
    this.numberOfIntersection = (object as StateVisObjConnection).numberOfIntersection;

    this.listOfConnectionsIntersection = (object as StateVisObjConnection).listOfConnectionsIntersection.toList(growable: false);
  }
  ...
  void preCalculateNeighboursValue([int startRange = 1, int endRange = -1]){
    ...
    var previousBlockIdAtNewPos, blockToMove;
    for(int neighbour = startRange; neighbour < endRange; neighbour++){
      ...
      m = this.groupChildrenNumber[groupToMoveIndex];
      ...
      if(m != 1){
        ...
        int blockToMoveIndex = insideGroup ~/ (m-1);
      ...
      /*groupToMove.childrenIterable.forEach((VisualObject block){
        block.childrenIterable.forEach((VisualObject bars){
          bars.connection.updateSegmentsRadianPos(this.posDoubleConvertValue, this.maxBlockNumberInGroup);
        });
      });

      groupAtNewPos.childrenIterable.forEach((VisualObject block){
        block.childrenIterable.forEach((VisualObject bars){
          bars.connection.updateSegmentsRadianPos(this.posDoubleConvertValue, this.maxBlockNumberInGroup);
        });
      });*/
  ...
  bool chooseNeighbourAndDecideToKeepByFunc(int neighbour, {Function functionToDecide = null, bool enablePreCalculate = true, int startRange = 1, int endRange = -1}) {
    ...
    groupToMoveIndex = index;
    groupFinalPosition = remainder ~/ m + 1;
    moveBy = index + groupFinalPosition;
  ...
  State clone(){
    ...
    return new StateVisObjConnection.simpleCopy(this);
  }
  ...
  void copySavedStateIntoAnother(State other, [bool finalState = false]){
    StateVisObjConnection otherState = (other as StateVisObjConnection);
