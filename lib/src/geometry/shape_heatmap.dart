...
class ShapeHeatmap extends ShapeForm {
  ...
  List<String> barsInBlockInOrder;

  Map<String, RangeMath<double>> barsRanges;
  ...
  ShapeHeatmap(this._lines, this._diagram, [this._parent = null]) : super._();

  ShapeHeatmap.fromData(this._diagram, RangeMath<double> range,
    ...
    var minRadius = this._diagram.segmentCircle.clone();
    minRadius.radius -= (2*this._diagram.lineWidth + (this._diagram.numberOfAdditionalLayer + 1) * 15);

    var minLineRadius = this._diagram.segmentCircle.clone();
    minLineRadius.radius -= (this._diagram.lineWidth + (this._diagram.numberOfAdditionalLayer + 1) * 15);

    var maxRadius = this._diagram.segmentCircle.clone();
    maxRadius.radius += this._diagram.lineWidth - (this._diagram.numberOfAdditionalLayer * 15);

    var maxLineRadius = this._diagram.segmentCircle.clone();
    maxLineRadius.radius -= (this._diagram.numberOfAdditionalLayer * 15);
  ...
  void modifyGeometry(RangeMath<double> rangeA, RangeMath<double> rangeB,
      ...
      /*var minRadius = this._diagram.baseCircle.radius - 2*this._diagram.lineWidth;

      var minLineRadius = this._diagram.baseCircle.radius - this._diagram.lineWidth;

      var maxRadius = this._diagram.drawCircle.radius + this._diagram.lineWidth;

      var maxLineRadius = this._diagram.drawCircle.radius;*/

      var minRadius = this._diagram.segmentCircle.radius - (2*this._diagram.lineWidth + (this._diagram.numberOfAdditionalLayer + 1) * 15);

      var minLineRadius = this._diagram.segmentCircle.radius - (this._diagram.lineWidth + (this._diagram.numberOfAdditionalLayer + 1) * 15);

      var maxRadius = this._diagram.segmentCircle.radius + this._diagram.lineWidth - (this._diagram.numberOfAdditionalLayer * 15);

      var maxLineRadius = this._diagram.segmentCircle.radius - (this._diagram.numberOfAdditionalLayer * 15);


      this._lines[0].lineArc.range = rangeA;
      this._lines[0].lineArc.circle.radius = minLineRadius;

      this._lines[1].lineArc.range = rangeA;
      this._lines[1].lineArc.circle.radius = maxLineRadius;

      this._lines[2].lineArc.range = this._innerRange;
      this._lines[2].lineArc.circle.radius = minRadius;

      this._lines[3].lineArc.range = this._innerRange;
      this._lines[3].lineArc.circle.radius = maxRadius;
  ...
  List<Vector3> generatePolygonData(){
    ...
    var endPoint = new Vector3.array(this._diagram.directionCircle.getPointFromPolarCoordinate(range.end).coordinate.storage as List<double>);*/
      ...
      var midPoint2 = ((midPoint - middle).normalized() * ((-directionIndicatorHeight*0.5) - this._diagram.lineWidth)) + midPoint;
