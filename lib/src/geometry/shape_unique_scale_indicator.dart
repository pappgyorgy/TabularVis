...
class ShapeUniqueScaleIndicator extends ShapeSimple {
  ...
  ShapeUniqueScaleIndicator(this._lines, this._diagram, [this._parent = null]) : super.empty();

  ShapeUniqueScaleIndicator.fromData(this._diagram, RangeMath<double> range,
    ...
    lineSegmentCircle = this._diagram.segmentCircle.clone()
      ..radius += (this._diagram.maxSegmentRadius + this._diagram.lineWidth);


    this._lines.add(new LineGeom(
        LineType.simple,
        this._diagram,
        this._diagram.segmentCircle.clone()..radius += 3*this._diagram.lineWidth,
        range
    ));
  ...
  void modifyGeometry(RangeMath<double> rangeA, RangeMath<double> rangeB,
      ...
      var lineSegmentCircleRadius = this._diagram.segmentCircle.clone().radius += (this._diagram.maxSegmentRadius + this._diagram.lineWidth);


      //segmentOuterCircleRadius += halfHeight;

      this._lines[2].lineArc.range = this._innerRange;
      this._lines[0].lineArc.circle = this._diagram.segmentCircle.clone()..radius += 3*this._diagram.lineWidth;
  ...
  List<Face3> generateFaceData(){
    ...
    Color whiteCol = new Color(0xffffff);
    List<Color> vertexColors = <Color>[whiteCol, whiteCol, whiteCol];
