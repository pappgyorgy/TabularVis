...
class ShapeEdgeBundle extends ShapeForm {
  ...
  RangeMath<double> blockRange;

  double groupMiddle;
  double groupMiddle2;

  double oppositeBlockMiddle;

  double connMidA, connMidB;

  bool _is3D = false;
  double shapeHeight = 10.0;

  ShapeEdgeBundle(this._lines, this._diagram) : super._();

  ShapeEdgeBundle.fromData(this._diagram,
      RangeMath<double> rangeOne,
      RangeMath<double> rangeTwo,
      [this._parent = null, String key = "",
      this._is3D = false, this.shapeHeight = 10.0, RangeMath<double> groupRange, RangeMath<double> groupRange2]) : super._(){

    this.blockRange = groupRange;

    this.connMidA = MathFunc.getMidRange(rangeOne.getMinValue(), rangeOne.getMaxValue());
    this.connMidB = MathFunc.getMidRange(rangeTwo.getMinValue(), rangeTwo.getMaxValue());

    this.groupMiddle = MathFunc.getMidRange(groupRange.getMinValue(), groupRange.getMaxValue());
    this.groupMiddle2 = MathFunc.getMidRange(groupRange2.getMinValue(), groupRange2.getMaxValue());

    this.oppositeBlockMiddle = this.groupMiddle + pi;
    ...
    this._lines[0] = new LineGeom(LineType.bezierExt,
      this._diagram, this._diagram.drawCircle, firstSideRange, null, null, this.groupMiddle, this.groupMiddle2, this.connMidA, this.connMidB, false);

    this._lines[1] = new LineGeom(LineType.bezierExt,
        this._diagram, this._diagram.drawCircle, secondSideRange, null, null, this.groupMiddle, this.groupMiddle2, this.connMidA, this.connMidB, true);
  ...
  List<Vector3> generatePolygonData(){
    ...
    for(var i = 0; i <= resRangesTwo.length; i++){
      ...
      bool value = rangeTwo.begin < rangeTwo.end ? i < (resRangesTwo.length ~/ 2) : i > (resRangesTwo.length ~/ 2);

      var newLine = new LineGeom(LineType.bezierExt,
          this._diagram, this._diagram.lineOuterDrawCircle, newLineRange, null,
          null, this.groupMiddle, this.groupMiddle2, this.connMidA, this.connMidB,
          value);
  ...
  void modifyGeometry(RangeMath<double> rangeA, RangeMath<double> rangeB,
    ...
    this.groupMiddle = MathFunc.getMidRange(blockRange.getMinValue(), blockRange.getMaxValue());
    this.groupMiddle2 = MathFunc.getMidRange(blockRange2.getMinValue(), blockRange2.getMaxValue());
    ...
    (this._lines[0] as LineBezierExt).groupMiddle = this.groupMiddle;
    (this._lines[0] as LineBezierExt).groupMiddle2 = this.groupMiddle2;
    (this._lines[0] as LineBezierExt).connMiddleA = this.connMidA;
    (this._lines[0] as LineBezierExt).connMiddleB = this.connMidB;

    this._lines[1].lineArc.range = secondSideRange;
    (this._lines[1] as LineBezierExt).groupMiddle = this.groupMiddle;
    (this._lines[1] as LineBezierExt).groupMiddle2 = this.groupMiddle2;
    (this._lines[1] as LineBezierExt).connMiddleA = this.connMidA;
    (this._lines[1] as LineBezierExt).connMiddleB = this.connMidB;
